...
<template>
    <div ref="dropdownRef" class="relative flex items-center gap-2 select-none">
        <!-- Globe Icon -->
        ...
        <div class="flex items-center gap-1.5 text-sm">
            <template v-for="(lang, idx) in languages" :key="lang.code">
                <button
                    ...
                    :class="[
                        'font-medium transition-all duration-200 px-0.5 py-0.5 rounded leading-none',
                        locale === lang.code
                ...
                <span v-if="idx < languages.length - 1" class="text-white/30 text-xs">|</span>
            </template>
        </div>
    </div>
</template>
