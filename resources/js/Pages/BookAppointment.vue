...
<template>
    <Head :title="($t('booking.title') || 'Book Appointment') + ' | Makkah Specialist Eye Hospital'" />
    ...
    <div v-if="!$page.props.settings?.enable_appointments" class="min-h-screen py-16 px-4 bg-slate-50/70 mt-8 flex items-center justify-center">
        <div class="max-w-2xl w-full bg-white rounded-3xl shadow-xl p-8 sm:p-12 text-center border border-gray-100" data-aos="fade-up" data-aos-duration="800">
            ...
            <h2 class="text-3xl font-black text-slate-900 mb-3 font-['Outfit',sans-serif]">{{ $t('booking.pausedTitle') || 'Online Booking Paused' }}</h2>
            <p class="text-slate-600 mb-6 max-w-md mx-auto leading-relaxed">
                {{ $t('booking.pausedMessage') || 'We are currently restructuring our scheduling system. Online appointment requests are temporarily unavailable.' }}
            </p>
            <div class="bg-slate-50 rounded-2xl p-5 mb-8 border border-gray-100 text-left space-y-3">
                <h4 class="font-bold text-slate-900 text-sm">{{ $t('booking.howToSchedule') || 'How to schedule your visit:' }}</h4>
                <p class="text-sm text-slate-600 flex items-center gap-2">
                    <svg class="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.94.725l.548 2.2a1 1 0 01-.321.988l-1.305.98a10.582 10.582 0 004.872 4.872l.98-1.305a1 1 0 01.988-.321l2.2.548a1 1 0 01.725.94V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                    {{ $t('booking.callUs') || 'Call us directly at:' }} <span class="font-bold text-slate-900">{{ $page.props.support?.phone_formatted || '(+234) 916 666 1264' }}</span>
                </p>
                <p class="text-sm text-slate-600 flex items-center gap-2">
                    ...
                    {{ $t('booking.emailUs') || 'Email us at:' }} <span class="font-bold text-slate-900">{{ $page.props.support?.email || 'makkah.ibadan@albasar.com' }}</span>
                </p>
            </div>
            <Link :href="route('contact')" class="inline-flex items-center gap-2 bg-primary hover:bg-[#0880cc] text-white font-extrabold px-8 py-3.5 rounded-full shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
                {{ $t('booking.goToContact') || 'Go to Contact Page' }}
            </Link>
        </div>
    </div>
    ...
    <div v-else class="min-h-screen py-16 px-4 bg-slate-50/70 mt-8">
        <div class="max-w-3xl md:max-w-4xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 hover:shadow-2xl transition-shadow duration-300" data-aos="fade-up" data-aos-duration="800">
            <div class="bg-gradient-to-r from-primary to-[#06465C] text-white p-8 md:p-10 text-center" v-show="!$page.props.flash.success_tracking">
                ...
                <h1 class="text-3xl sm:text-4xl font-black font-['Outfit',sans-serif]">{{ $t('booking.requestTitle') || 'Request an Appointment' }}</h1>
                <p class="mt-2 text-white/90 text-sm sm:text-base">{{ $t('booking.requestSubtitle') || 'Schedule your visit with our world-class ophthalmic specialists.' }}</p>
            </div>
            ...
            <div class="p-6 md:p-10" v-if="$page.props.flash.success_tracking">
                <div class="text-center py-8">
                    ...
                    <h3 class="text-2xl font-black text-slate-900 font-['Outfit',sans-serif]">{{ $t('booking.receivedTitle') || 'Appointment Request Received!' }}</h3>
                    <p class="text-slate-600 mt-2 max-w-md mx-auto">
                        {{ $t('booking.trackingNotice') || 'Thank you for booking with us. Your tracking code is:' }}
                    </p>
                    ...
                    <p class="text-xs text-slate-500 mt-4 max-w-sm mx-auto">
                        {{ $t('booking.saveNotice') || 'Please save this tracking code to check your appointment status anytime.' }}
                    </p>
            ...
            <div class="p-6 md:p-10" v-else>
                <form @submit.prevent="submitAppointment" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <TextInput name="first_name" :label="$t('booking.form.firstName') || 'First Name'" v-model="form.first_name" type="text" placeholder="Ali" :message="form.errors.first_name" required />
                        <TextInput name="last_name" :label="$t('booking.form.lastName') || 'Last Name'" v-model="form.last_name" type="text" placeholder="Ahmed" :message="form.errors.last_name" required />
                        <TextInput name="email" :label="$t('booking.form.email') || 'Email Address'" v-model="form.email" type="email" placeholder="ali@email.com" :message="form.errors.email" required />
                        <TextInput name="phone" :label="$t('booking.form.phone') || 'Phone Number'" v-model="form.phone" type="text" placeholder="+234 800 123 456" :message="form.errors.phone" required />
                        
                        <!-- Details -->
                        <TextInput name="dob" :label="$t('booking.form.dob') || 'Date of Birth'" v-model="form.dob" type="date" :message="form.errors.dob" required />
                        
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">{{ $t('booking.form.serviceNeeded') || 'Service Needed' }}</label>
                        ...
                        <!-- Date and Time -->
                        <TextInput name="appointment_date" :label="$t('booking.form.preferredDate') || 'Preferred Date'" v-model="form.appointment_date" type="date" :message="form.errors.appointment_date" required />
                        <TextInput name="appointment_time" :label="$t('booking.form.preferredTime') || 'Preferred Time'" v-model="form.appointment_time" type="time" :message="form.errors.appointment_time" required />
                    </div>
                    ...
                    <div class="mt-4">
                        <label class="flex items-center space-x-3 cursor-pointer">
                            ...
                            <span class="text-sm text-slate-700 font-semibold">{{ $t('booking.form.existingPatient') || 'I am an existing patient' }}</span>
                        </label>
                    </div>

                    <!-- File Number Field if existing patient -->
                    <div class="mt-4" v-if="form.is_existing_patient">
                        <TextInput name="file_number" :label="$t('booking.form.fileNumber') || 'File Number'" v-model="form.file_number" type="text" placeholder="Enter your hospital file number" :message="form.errors.file_number" required />
                    </div>

                    <div class="mt-4">
                        <label class="block text-sm font-bold text-slate-700 mb-1">{{ $t('booking.form.notes') || 'Any symptoms or notes?' }}</label>
                    ...
                    <div class="pt-4 flex justify-end">
                        <button type="submit" class="bg-primary hover:bg-[#0880cc] text-white font-extrabold py-3.5 px-10 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 cursor-pointer" :disabled="form.processing">
                            <span v-if="form.processing">{{ $t('booking.form.submitting') || 'Submitting...' }}</span>
                            <span v-else>{{ $t('booking.form.submit') || 'Confirm Request' }}</span>
                        </button>
                    </div>
                </form>
            </div>
