    ...
    <div>
        ...
        <section class="bg-gradient-to-br from-primary/10 via-blue-50 to-white dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-20 pt-36 md:pt-44">
            <div class="max-w-4xl mx-auto px-4 text-center">
                <span class="inline-block bg-primary/10 text-primary text-sm font-semibold px-4 py-1 rounded-full mb-4">{{ $t('testimonials.badge') }}</span>
                <h1 class="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-4" style="font-family: Georgia, serif;">{{ $t('testimonials.title') }}</h1>
                <p class="text-lg text-gray-500 dark:text-gray-400 mb-8">{{ $t('testimonials.subtitle') }}</p>
        ...
        <div v-if="submitted" class="max-w-2xl mx-auto px-4 py-6">
            <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-6 text-center">
                ...
                <h3 class="text-xl font-bold text-green-700 dark:text-green-400 mb-1">{{ $t('testimonials.submittedTitle') }}</h3>
                <p class="text-green-600 dark:text-green-500">{{ $t('testimonials.submittedMessage') }}</p>
            </div>
        </div>
            ...
            <div class="max-w-2xl mx-auto px-4">
                <div class="bg-gray-50 dark:bg-gray-800 rounded-2xl p-8 shadow-sm border border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('testimonials.formTitle') }}</h2>
                    ...
                    <form @submit.prevent="submit" class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">{{ $t('testimonials.yourName') }} *</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">{{ $t('testimonials.yourOccupation') }}</label>
                        ...
                        <!-- Rating -->
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ $t('testimonials.yourRating') }} *</label>
                            <div class="flex items-center gap-1">
                                ...
                                <span class="ml-3 text-sm font-medium text-gray-600 dark:text-gray-400">{{ $t('testimonials.ratings.' + (hoverRating || form.rating)) }}</span>
                            </div>
                        </div>
 
                        <!-- Message -->
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">{{ $t('testimonials.yourTestimony') }} *</label>
                            <textarea v-model="form.message" rows="5" required :placeholder="$t('testimonials.formPlaceholder')" class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-primary/40 focus:outline-none text-sm resize-none"></textarea>
        ...
        <section class="py-16 bg-gray-50 dark:bg-gray-800">
            <div class="max-w-7xl mx-auto px-4">
                ...
                <div v-if="!testimonials?.data?.length" class="flex flex-col items-center justify-center py-24 gap-4">
                    ...
                    <p class="text-xl font-semibold text-gray-500 dark:text-gray-400">{{ $t('testimonials.noTestimonials') }}</p>
