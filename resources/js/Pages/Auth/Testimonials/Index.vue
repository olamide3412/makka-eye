...
<template>
    <AuthenticatedLayout>
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Testimonials</h1>
            <p class="text-sm text-gray-500 mt-0.5">Review and approve patient testimonials before they appear publicly.</p>
        </div>

        <!-- Stats Row -->
        <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border-l-4 border-yellow-500 border border-gray-100 dark:border-gray-700">
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Pending</p>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ counts?.pending || 0 }}</p>
            </div>
            <div class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border-l-4 border-green-500 border border-gray-100 dark:border-gray-700">
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Approved</p>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ counts?.approved || 0 }}</p>
            </div>
            <div class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border-l-4 border-blue-500 border border-gray-100 dark:border-gray-700">
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total</p>
                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ counts?.all || 0 }}</p>
            </div>
        </div>
        ...
        <div class="flex gap-2 mb-4">
            <button v-for="f in [{ val: '', label: 'All' }, { val: 'pending', label: 'Pending' }, { val: 'approved', label: 'Approved' }, { val: 'rejected', label: 'Rejected' }]" :key="f.val"
                ...
                :class="['px-4 py-2 rounded-lg text-sm font-medium transition-colors border', statusFilter === f.val ? 'bg-primary text-white border-primary shadow' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:border-primary hover:text-primary']">
                {{ f.label }}
            </button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            <div v-for="t in testimonials?.data" :key="t.id"
                class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
                <!-- Header -->
                ...
                <div class="flex gap-0.5 mb-3">
                    <svg v-for="n in 5" :key="n" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" :class="n <= t.rating ? 'text-yellow-400' : 'text-gray-200 dark:text-gray-700'">
                        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                    </svg>
                </div>

                <!-- Message -->
                <p class="text-sm text-gray-600 dark:text-gray-300 italic flex-1 mb-4 line-clamp-4">"{{ t.message }}"</p>
                ...
                <div class="flex gap-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                    <button v-if="t.status !== 'approved'" @click="approve(t.id)" :disabled="approveForm.processing"
                        class="flex-1 flex items-center justify-center gap-1.5 bg-green-50 hover:bg-green-100 dark:bg-green-900/20 dark:hover:bg-green-900/40 text-green-700 dark:text-green-400 font-medium text-xs px-3 py-2 rounded-lg transition-colors disabled:opacity-60">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        Approve
                    </button>
                    <button v-if="t.status !== 'rejected'" @click="reject(t.id)" :disabled="rejectForm.processing"
                        class="flex-1 flex items-center justify-center gap-1.5 bg-yellow-50 hover:bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 font-medium text-xs px-3 py-2 rounded-lg transition-colors disabled:opacity-60">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        Reject
                    </button>
                    <button @click="destroy(t.id)" :disabled="deleteForm.processing"
                        class="p-2 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg transition-colors disabled:opacity-60">
